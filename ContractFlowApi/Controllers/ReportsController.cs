...
public class ReportsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetDueDeliverables([FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Aggregates deliverables by supplier and returns counts for each
    /// supplier. Optional from/to query parameters filter by expected
    /// delivery date.
    /// </summary>
    [HttpGet("deliveries-by-supplier")]
    public async Task<IActionResult> GetDeliveriesBySupplier([FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Aggregates deliverables by organisational unit. Optional date
    /// filtering is supported via from/to query parameters.
    /// </summary>
    [HttpGet("deliveries-by-orgunit")]
    public async Task<IActionResult> GetDeliveriesByOrgUnit([FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Returns a list of penalties applied. Optionally filter by the
    /// registration date of the non‑compliance via from/to query
    /// parameters.
    /// </summary>
    [HttpGet("penalties")]
    public async Task<IActionResult> GetPenalties([FromQuery] DateTime? from, [FromQuery] DateTime? to, CancellationToken ct)
    {
